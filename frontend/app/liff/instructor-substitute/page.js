...
function InstructorSubstituteContent() {
  ...
  // 代行募集一覧表示
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-teal-50 py-4 px-4">
            ...
            {weekDays.map((day) => {
              ...
              return (
                <div key={dateStr} className="bg-white rounded-xl shadow overflow-hidden">
                  <div className={`px-4 py-2 border-b ${isToday ? 'bg-emerald-50' : 'bg-gray-50'}`}>
                    <h3 className={`font-medium ${isToday ? 'text-emerald-700' : 'text-gray-900'}`}>
                      {format(day, 'M月d日', { locale: ja })}
                      <span className={`ml-1 ${dayOfWeek === 0 ? 'text-red-500' : dayOfWeek === 6 ? 'text-blue-500' : ''}`}>
                        ({DAYS_OF_WEEK[dayOfWeek]})
                      </span>
                      {isToday && <span className="ml-2 text-xs bg-emerald-500 text-white px-2 py-0.5 rounded">今日</span>}
                    </h3>
                  </div>
...
export default function LiffInstructorSubstitutePage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-b from-emerald-50 to-teal-50 flex items-center justify-center">
        <div className="text-center">
