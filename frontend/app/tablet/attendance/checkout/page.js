...
export default function TabletAttendanceCheckoutPage() {
  ...
  useEffect(() => {
    ...
    const initScanner = async () => {
      try {
        ...
        html5QrCode = new Html5Qrcode('qr-reader-attendance-checkout')
  ...
  const onScanSuccess = useCallback(async (decodedText) => {
    ...
    if (!memberMatch && !staffMatch) return
    ...
    try {
      if (memberMatch) {
        ...
        await handleMemberCheckout(qrToken)
      } else if (staffMatch) {
        const qrToken = staffMatch[1]
        await handleStaffCheckout(qrToken)
      }
  ...
  const handleMemberCheckout = async (qrToken) => {
    ...
    if (!scanData.current_visit) {
      throw new Error('入館記録がありません。入館画面から入館してください。')
    }

    const checkoutRes = await fetch('/api/tablet/checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (!checkoutRes.ok) {
      throw new Error(checkoutData.error || '退館処理に失敗しました')
    }
  ...
  const handleStaffCheckout = async (qrToken) => {
    ...
    // 出勤中かチェック
    if (!scanData.today_attendance?.clock_in_at) {
      throw new Error('出勤記録がありません。出勤画面から出勤してください。')
    }
    ...
    const checkoutRes = await fetch('/api/tablet/staff-checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    if (!checkoutRes.ok) {
      throw new Error(checkoutData.error || '退勤処理に失敗しました')
    }

    setMember(null)
    setStaff(staffData)
    setWorkMinutes(checkoutData.actual_work_minutes || 0)
    setScanState(SCAN_STATE.SUCCESS)
  }
  ...
  if (isAuthChecking) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-orange-900 to-amber-900 text-white flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-900 to-amber-900 text-white">
      {/* ヘッダー */}
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <LogOutIcon className="w-6 h-6 text-orange-400" />
              <span className="text-xl font-bold">退館</span>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="bg-white/10 border border-white/20 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
              >
                {stores.map((store) => (
      ...
      <main className="max-w-4xl mx-auto p-6">
        {(scanState === SCAN_STATE.WAITING || scanState === SCAN_STATE.SCANNING) && (
          <div className="text-center">
            <h1 className="text-4xl font-bold mb-2">退館</h1>
            <p className="text-orange-200 text-xl mb-8">QRコードをかざしてください</p>

            <div className="relative inline-block">
              <div
                id="qr-reader-attendance-checkout"
            ...
            {scanState === SCAN_STATE.WAITING && (
              <div className="mt-6 flex items-center justify-center gap-2 text-orange-300">
                <Loader2 className="w-5 h-5 animate-spin" />
                カメラを起動中...
              </div>
            )}

            <div className="mt-6 text-sm text-orange-300/70">
        ...
        {scanState === SCAN_STATE.SUCCESS && member && (
          <div className="text-center">
            <div className="w-40 h-40 bg-orange-500 rounded-full flex items-center justify-center mx-auto mb-8 animate-pulse">
              <CheckCircle className="w-24 h-24 text-white" />
            </div>
            <h1 className="text-5xl font-bold mb-4">お疲れ様でした！</h1>
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 max-w-md mx-auto">
              <div className="flex items-center justify-center gap-3 mb-2">
                <User className="w-8 h-8 text-orange-300" />
                <span className="text-3xl font-bold">{member.name}</span>
                <span className="text-orange-300">さん</span>
              </div>
              <p className="text-orange-200">会員番号: {member.member_number}</p>
            </div>
          </div>
        )}
          ...
          <div className="text-center">
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 max-w-md mx-auto">
              ...
              <p className="text-blue-300 mt-2">
                {currentTime.toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' })} 退勤
              </p>
              {workMinutes > 0 && (
                <p className="text-blue-200 mt-1">
                  勤務時間: {formatWorkTime(workMinutes)}
                </p>
              )}
            </div>
          </div>
