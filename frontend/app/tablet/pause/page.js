...
function TabletPauseContent() {
  ...
  const fetchMemberById = async (memberId) => {
    try {
      ...
      if (memberData.status === 'paused') {
        setError('この会員は既に休会中です')
        setStep('verify')
        return
      }
      if (memberData.status === 'canceled') {
        setError('この会員は退会済みです')
        setStep('verify')
        return
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/tablet/pause', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          member_id: member.id,
          reason: formData.reason,
          paused_until: formData.paused_until || null,
        }),
      })
      ...
      if (!res.ok) {
        setError(data.error || '休会処理に失敗しました')
        return
      }

      setStep('complete')
    } catch (err) {
      setError('休会手続きに失敗しました')
    } finally {
      setIsProcessing(false)
    }
  }
      ...
      <div className="min-h-screen bg-gradient-to-b from-violet-50 to-purple-50 py-12 px-4">
        <div className="max-w-lg mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-8 text-center">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mb-4">休会手続き完了</h1>
            <p className="text-lg text-gray-600 mb-8">
              休会手続きが完了しました。
              <br />
              休会中は月額料金の請求が停止されます。
              <br /><br />
              復帰をご希望の際は、スタッフにお声がけください。
            </p>
            {isFromQR ? (
              <a
              ...
              <button
                onClick={() => {
                  ...
                  setFormData({ reason: '', paused_until: '' })
                }}
      ...
      <div className="min-h-screen bg-gradient-to-b from-violet-50 to-purple-50 py-12 px-4">
        <div className="max-w-lg mx-auto">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl font-bold text-gray-900 mt-6">休会手続き</h1>
      ...
      <div className="max-w-lg mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5 mb-6">
            <h3 className="font-bold text-yellow-800 flex items-center gap-2 mb-3 text-lg">
              <AlertTriangle className="w-6 h-6" />
              休会についてのご案内
            </h3>
            <ul className="text-yellow-700 space-y-2">
              <li>・ 休会中は月額料金の請求が停止されます</li>
              <li>・ 休会中はレッスンの予約ができません</li>
              <li>・ 復帰はスタッフにお声がけいただくか、マイページから可能です</li>
              <li>・ 復帰した翌月から請求が再開されます</li>
            </ul>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              ...
              <input
                type="date"
                value={formData.paused_until}
                onChange={(e) => setFormData({ ...formData, paused_until: e.target.value })}
                min={new Date().toISOString().split('T')[0]}
                className="w-full px-5 py-4 text-xl border-2 border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-violet-500"
              />
            ...
            <div className="flex gap-4">
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="flex-1 py-4 bg-yellow-600 text-white rounded-xl text-lg font-medium hover:bg-yellow-700 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {isProcessing ? (
                  <>
                    <Loader2 className="w-6 h-6 animate-spin" />
...
export default function TabletPausePage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-gradient-to-b from-violet-50 to-purple-50 py-12 px-4">
        <div className="max-w-lg mx-auto">
