...
import { ArrowLeft, UserX, QrCode, Loader2 } from 'lucide-react'

const SESSION_KEY = 'tablet_procedures_session'

export default function TabletProceduresCancelPage() {
  ...
  const handleStart = () => {
    ...
    if (sessionData) {
      const session = JSON.parse(sessionData)
      router.push(`/tablet/procedures/scan?procedure=cancel&token=${session.token}`)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      <div className="max-w-lg mx-auto py-12 px-4">
        ...
        <div className="bg-white/10 rounded-3xl p-8 text-center mb-8">
          ...
          <h2 className="text-2xl font-bold mb-2">退会手続き</h2>
          <p className="text-gray-400">会員契約を解約します</p>
        </div>
        ...
        <div className="space-y-4">
          <button
            onClick={handleStart}
            className="w-full py-5 rounded-xl text-xl font-bold transition-all bg-red-600 hover:bg-red-700"
          >
            開始する
          </button>
          <button
