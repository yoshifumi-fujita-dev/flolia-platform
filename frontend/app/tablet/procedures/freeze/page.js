...
import { ArrowLeft, PauseCircle, QrCode, Loader2 } from 'lucide-react'

const SESSION_KEY = 'tablet_procedures_session'

export default function TabletProceduresFreezePage() {
  ...
  const handleStart = () => {
    ...
    if (sessionData) {
      const session = JSON.parse(sessionData)
      router.push(`/tablet/procedures/scan?procedure=freeze&token=${session.token}`)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      <div className="max-w-lg mx-auto py-12 px-4">
        ...
        <div className="bg-white/10 rounded-3xl p-8 text-center mb-8">
          <div className="w-20 h-20 bg-yellow-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <PauseCircle className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-2xl font-bold mb-2">休会手続き</h2>
          <p className="text-gray-400">一時的に契約を休止します</p>
        </div>
        ...
        <div className="space-y-4">
          <button
            onClick={handleStart}
            className="w-full py-5 rounded-xl text-xl font-bold transition-all bg-yellow-600 hover:bg-yellow-700"
          >
            開始する
          </button>
          <button
