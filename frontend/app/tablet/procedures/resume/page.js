...
import { ArrowLeft, PlayCircle, QrCode, Loader2 } from 'lucide-react'

const SESSION_KEY = 'tablet_procedures_session'

export default function TabletProceduresResumePage() {
  ...
  const handleStart = () => {
    ...
    if (sessionData) {
      const session = JSON.parse(sessionData)
      router.push(`/tablet/procedures/scan?procedure=resume&token=${session.token}`)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 text-white">
      <div className="max-w-lg mx-auto py-12 px-4">
        ...
        <div className="bg-white/10 rounded-3xl p-8 text-center mb-8">
          ...
          <h2 className="text-2xl font-bold mb-2">復帰手続き</h2>
          <p className="text-gray-400">休会中の会員を復帰させます</p>
        </div>
        ...
        <div className="space-y-4">
          <button
            onClick={handleStart}
            className="w-full py-5 rounded-xl text-xl font-bold transition-all bg-green-600 hover:bg-green-700"
          >
            開始する
          </button>
          <button
